	...
	*/
	public class b2Body
	{
		...
		public function CreateFixture(def:b2FixtureDef):b2Fixture
		{
			...
			if(hasFlag(e_activeFlag)){
				fixture.CreateProxy(m_world.m_contactManager.m_broadPhase, m_xf);
			}
			...
			m_world.addFlag(b2World.e_newFixture);
		...
		public function DestroyFixture(fixture:b2Fixture):void
		{
			...
			var ppF:b2Fixture; // Fix pointer-pointer stuff
			while(node)
			{
				if (node == fixture)
				{
		...
		*/
		public function SetPositionAndAngle(position:b2Vec2, angle:Number):void
		{
			...
			for (var f:b2Fixture = m_fixtureList; f; f = f.next){
		...
		*/
		...
		public function GetDefinition():b2BodyDef
		{
			...
			bd.type = GetType();
			bd.allowSleep = hasFlag(e_allowSleepFlag);
			bd.angle = GetAngle();
			bd.angularDamping = m_angularDamping;
			bd.angularVelocity = m_angularVelocity;
			bd.fixedRotation = hasFlag(e_fixedRotationFlag);
			bd.bullet = hasFlag(e_bulletFlag);
			bd.awake = hasFlag(e_awakeFlag);
		...
		*/
		...
		{
			for(var f:b2Fixture = other.m_fixtureList; f;){
				var next:b2Fixture = f.next;
		...
		*/
		...
		public function SetMassData(massData:b2MassData):void
		{
			...
			if (massData.I > 0.0 && hasFlag(e_fixedRotationFlag) == false)
//			if (massData.I > 0.0 && (m_flags & e_fixedRotationFlag) == 0)
		...
		public function ResetMassData():void
		{
			...
			if (m_I > 0.0 && hasFlag(e_fixedRotationFlag) == false)
//			if (m_I > 0.0 && (m_flags & e_fixedRotationFlag) == 0)
		...
		*/
		...
		{
			if(flag){
				addFlag(e_bulletFlag);
			}else{
				clearFlag(e_bulletFlag);
			}
		}
		...
		*/
		...
		{
			if(flag){
				addFlag(e_allowSleepFlag);
			}else{
				clearFlag(e_allowSleepFlag);
				SetAwake(true);
			}
		}
		...
		{
			if(fixed){
				addFlag(e_fixedRotationFlag);
			}else{
				clearFlag(e_fixedRotationFlag);
			}
		...
		*/
		...
		{
			return hasFlag(e_fixedRotationFlag);
		}
		...
		*/
		public function SetActive(flag:Boolean):void
		{
			...
			if(flag)
			{
				addFlag(e_activeFlag);
	
				// Create all proxies.
				for(f=m_fixtureList; f; f=f.next){
		...
		*/
		...
		{
			return hasFlag(e_allowSleepFlag);
		}
		...
		*/
		...
		static private const s_xf1:b2Transform = new b2Transform();
		...
		//{
			...
			static b2internal const e_allowSleepFlag:uint		= 0x0004;
		...
		//{
			...
			static public const b2_staticBody:uint = 0;
			static public const b2_kinematicBody:uint = 1;
			static public const b2_dynamicBody:uint = 2;
		//};
