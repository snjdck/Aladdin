	...
	public class b2World
	{
		...
		*/
		...
		public function SetBroadPhase(broadPhase:IBroadPhase) : void {
			...
			{
				for (var f:b2Fixture = b.m_fixtureList; f; f = f.next)
				{
...
//			if(m_flags & e_newFixture)
			if(hasFlag(e_newFixture))
			{
				m_contactManager.FindNewContacts();
//				m_flags &= ~e_newFixture;
				clearFlag(e_newFixture);
			}
...
//			m_flags &= ~e_locked;
		...
		public function DrawDebugData() : void{
			...
			if (flags & b2DebugDraw.e_shapeBit)
			{
				...
				{
					xf = b.m_xf;
					for (f = b.GetFixtureList(); f; f = f.next)
					{
						s = f.GetShape();
						if (b.IsActive() == false)
						{
			...
			if (flags & b2DebugDraw.e_pairBit)
			{
				...
				for (var contact:b2Contact = m_contactManager.m_contactList; contact; contact = contact.GetNext())
				{
					...
					var cA:b2Vec2 = fixtureA.m_aabb.GetCenter();
					var cB:b2Vec2 = fixtureB.m_aabb.GetCenter();
...
//			return (m_flags & e_locked) > 0;
		...
		b2internal function Solve(step:b2TimeStep) : void{
			...
			for (b = m_bodyList; b; b = b.m_next)
			{
				b.clearFlag(b2Body.e_islandFlag);
	//			b.m_flags &= ~b2Body.e_islandFlag;
			}
			for (var c:b2Contact = m_contactList; c; c = c.m_next)
			{
				c.clearFlag(b2Contact.e_islandFlag);
//				c.m_flags &= ~b2Contact.e_islandFlag;
	...
	//			if (seed.m_flags & b2Body.e_islandFlag )
				if (seed.hasFlag(b2Body.e_islandFlag))
				{
					continue;
				}
				...
				seed.addFlag(b2Body.e_islandFlag);
	//			seed.m_flags |= b2Body.e_islandFlag;
				...
				while (stackCount > 0)
				{
					...
					for (var ce:b2ContactEdge = b.m_contactList; ce; ce = ce.next)
					{
						...
						if (ce.contact.hasFlag(b2Contact.e_islandFlag))
						{
							continue;
						}
						...
						ce.contact.addFlag(b2Contact.e_islandFlag);
//						ce.contact.m_flags |= b2Contact.e_islandFlag;
	...
	//					if (other.m_flags & b2Body.e_islandFlag)
						if (other.hasFlag(b2Body.e_islandFlag)){
							continue;
						}
						
						//b2Settings.b2Assert(stackCount < stackSize);
						stack[stackCount++] = other;
						other.addFlag(b2Body.e_islandFlag);
	...
	//			b.m_flags &= ~b2Body.e_islandFlag;
			...
			{
				// Invalidate TOI
				c.clearFlag(b2Contact.e_toiFlag | b2Contact.e_islandFlag);
...
//					if (c.m_flags & b2Contact.e_toiFlag)
					if (c.hasFlag(b2Contact.e_toiFlag))
					{
...
//				minContact.m_flags &= ~b2Contact.e_toiFlag;
				minContact.clearFlag(b2Contact.e_toiFlag);
...
//						if (cEdge.contact.m_flags & b2Contact.e_islandFlag)
						if (cEdge.contact.hasFlag(b2Contact.e_islandFlag))
						{
							continue;
						}
...
//						cEdge.contact.m_flags |= b2Contact.e_islandFlag;
						cEdge.contact.addFlag(b2Contact.e_islandFlag);
...
//						cEdge.contact.m_flags &= ~b2Contact.e_toiFlag;
						cEdge.contact.clearFlag(b2Contact.e_toiFlag);
					}
				}
...
//					c.m_flags &= ~(b2Contact.e_toiFlag | b2Contact.e_islandFlag);
		...
		static private const s_jointColor:b2Color = new b2Color(0.5, 0.8, 0.8);
		//
		...
		b2internal const m_contactManager:b2ContactManager = new b2ContactManager();
		
		// These two are stored purely for efficiency purposes, they don't maintain
		// any data outside of a call to Step
		private const m_contactSolver:b2ContactSolver = new b2ContactSolver();
		private const m_island:b2Island = new b2Island();
		...
		// m_flags
		static public const e_newFixture:int = 0x0001;
		static public const e_locked:int = 0x0002;
	}
}