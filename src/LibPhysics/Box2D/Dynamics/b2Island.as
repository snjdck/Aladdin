	...
	public class b2Island
	{
		...
		public function Solve(step:b2TimeStep, gravity:b2Vec2, allowSleep:Boolean):void
		{
			...
			for(i=0; i<m_bodyCount; i++)
			{
				...
				if (rotation * rotation > b2Settings.b2_maxRotationSquared){
					...
					if (b.m_angularVelocity < 0){
						b.m_angularVelocity = -b.m_angularVelocity;
					}
				}
			...
			for(i=0; i<step.positionIterations; i++){
				...
				for(j=0; j<m_jointCount; j++){
					joint = m_joints[j];
					jointsOkay &&= joint.SolvePositionConstraints(b2Settings.b2_contactBaumgarte);
				}
			...
			for(i=0; i<m_bodyCount; i++)
			{
				...
				if(
					b.hasFlag(b2Body.e_allowSleepFlag) == false ||
//					0 == (b.m_flags & b2Body.e_allowSleepFlag) ||
		...
		static private const s_impulse:b2ContactImpulse = new b2ContactImpulse();
		public function Report(constraints:Vector.<b2ContactConstraint>):void
		{
			...
			for(var i:int=0; i<m_contactCount; i++){
				...
				m_listener.PostSolve(m_contacts[i], s_impulse);
			}
		}
